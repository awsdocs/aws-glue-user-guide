...
**Topics**
+ [Accessing the job graph editor](#edit-nodes-access-editor)
+ [Editing the data source node](#edit-jobs-source)
+ [Editing the data transform node](#edit-jobs-transforms)
+ [Configuring data target nodes](#data-target-nodes)
+ [Adding nodes to the job graph](#edit-job-add-nodes)
+ [Changing the parent nodes for a node in the job graph](#edit-job-change-parents)
+ [Deleting nodes from the job graph](#edit-job-delete-node)
...
1. Choose the **Node properties** tab in the node details panel, then enter the following information:
...
+ [Renaming a property key](#transforms-configure-rename-field)
...
+ **Custom transform**: Enter code into a text entry field to use custom transforms\. The output can be either a single `DynamicFrame` or a collection of `DynamicFrames`\.
...
### Renaming a property key<a name="transforms-configure-rename-field"></a>
...
1. If there is only one data source available, you must add a new data source node to the graph\. See [Adding nodes to the job graph](#edit-job-add-nodes) for details\.
...
If your custom code creates a collection of `DynamicFrames` as the output, then you must use a *SelectFromCollection* transform to choose a single `DynamicFrame` from the result before you can send the output to a target location\.
...
You can type or copy code into an input field\. The job uses this code to perform the data transformation\. You can provide a code snippet in either Python or Scala\. The code should take one or more `DynamicFrames` as input and return either a `DynamicFrame` or a collection of `DynamicFrames`\.
...
A custom code node can have any number of parent nodes, each providing a `DynamicFrame` as input for your custom code\. A custom code node can return either a single `DynamicFrame` or a collection of `DynamicFrames`\. Each `DynamicFrame` that is used as input has an associated schema\. If your code returns a collection of `DynamicFrame`, then you must add a schema that describes each `DynamicFrame` returned by the custom code node\.
...
1. If your custom transform code splits the datasets into multiple DyamicFrames, then choose the ![\[\]](http://docs.aws.amazon.com/glue/latest/ug/images/edit-schema-actions-button.png) button and then choose either **Add output schema** or **Duplicate**\. Then add new root keys to the schema or edit the duplicated keys\.
...
1. Edit the node, as described in the following sections:
   + For a source node, see [Editing the data source node](#edit-jobs-source)\.
   + For a transform node, see [Editing the data transform node](#edit-jobs-transforms)\.
   + For a data target node, see [Editing the data target node](#edit-jobs-target)\.
...
## Changing the parent nodes for a node in the job graph<a name="edit-job-change-parents"></a>
...
## Deleting nodes from the job graph<a name="edit-job-delete-node"></a>
